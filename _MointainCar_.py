# ========================================================================
# Autor: Izabela de Vasconcelos Granjeiro
# Curso: Engenharia de Sistemas Ciber-Físicos 
# Solução do problema do Mountain Car utilizando algoritmo de DQN
# ========================================================================
...
class MountainCarDQL():
    ...
    def train(self, episodes, render=False):
        ...
        #file = open(os.path.join('./save/', 'train_save'), 'w') 
        file = open('train_save.txt', 'w')
        ...
        for i in range(episodes):
            ...
            if i % 100 == 0:
                ...
                #print(f'final: {state[0]}\nmax: {positions[i]}')
                #file.write(f'{i}\t{rewards}\t{np.mean(rewards)}\n')  ###################################################################################################################################
            ...
            avg2 = np.mean(rewards_per_episode[-100:])
            avgs2.append(avg2)

            avg3 = np.mean(rewards_per_episode)
            avgs3.append(avg3)
            ...
            # Save the model at episode 17000
            if i == 17000:
                torch.save(policy_dqn.state_dict(), "mountaincar_dql_17000.pt")
        ...
        file.write('Ep \tReward \tAvg \tAvg_per_100_eps \tmax_pos \tmax_vel\n') ##############################################################################

        avg_100 = -1000
        f = 0
        while f < len(rewards_per_episode):
            ep= f+1
            r = rewards_per_episode[f]
            
            if f % 100 == 0:
                #print(int(f/100))
                avg_100 = avgs[int(f/100)]
            file.write(f'{ep}\t{r}\t{avgs3[f]}\t{avgs2[f]}\t{positions[f]}\t{velocities[f]}\n')
            #print(f'{ep}\t{r}\n')
            f += 1
...
if __name__ == '__main__':
    ...
    #mountaincar.test(10, "mountaincar_dql_10030.pt")